...
export async function POST(request: Request) {
  ...
  const { name, conditionField, conditionOperator, conditionValue, setCategory, setNotes, setTransfer, setHidden } =
    body as Record<string, unknown>;

  if (!name || typeof name !== "string" || !name.trim()) {
    return NextResponse.json({ error: "name is required" }, { status: 400 });
  }
  if (!conditionField || typeof conditionField !== "string" || !ALLOWED_FIELDS.has(conditionField)) {
    return NextResponse.json({ error: "invalid conditionField" }, { status: 400 });
  }
  if (!conditionOperator || typeof conditionOperator !== "string" || !ALLOWED_OPERATORS.has(conditionOperator)) {
